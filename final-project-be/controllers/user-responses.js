import { logger } from "../common/pino"; // Keep as ES6 import if you are using ES modules
...
export const getUserResponseController = (req, res) => { // Removed TypeScript types
  UserReponse.find({ documentId: req.params.documentId }, { _id: 0, answers: 0, __v: 0 }).populate({
    path: 'userId',
    model: 'User',
    select: 'username _id'
  })
    .then((responseData) => { // Removed TypeScript type
      res.status(200).send({
...
export const fetchUserResponseData = (req, res) => { // Removed TypeScript types
  UserReponse.find({ documentId: req.params.documentId, userId: req.params.userId }, { _id: 0, __v: 0 })
    ...
    .populate({
      ...
      model: 'User',
      select: 'username -_id'
    }).then((response) => { // Removed TypeScript type
      response = response[0];
      let data = {
...
export const saveUserResponseController = (req, res) => { // Removed TypeScript types
  ...
  UserReponse.findOneAndUpdate({ userId: req.body.userId, documentId: req.body.documentId },
    { $set: userResponse },
    { upsert: true, returnOriginal: false }).then((formResponse) => { // Removed TypeScript type
